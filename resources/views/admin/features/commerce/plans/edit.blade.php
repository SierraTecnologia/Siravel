...
@section('content')
    ...
    <div class="col-md-12 mt-4">
        <div class="row">
            ...
            <div class="col-md-6">
                {!! Form::model($plan, ['route' => [config('siravel.backend-route-prefix', 'siravel').'.plans.update', $plan->id], 'method' => 'patch']) !!}
                ...
                @if ($plan->enabled)
                    <a href="{{ url(config('siravel.backend-route-prefix', 'siravel').'/plans/'.$plan->id.'/state-change/disable') }}" class="btn btn-warning pull-right raw-margin-right-16">Disable</a>
                @else
                    <a href="{{ url(config('siravel.backend-route-prefix', 'siravel').'/plans/'.$plan->id.'/state-change/enable') }}" class="btn btn-outline-success pull-right raw-margin-right-16">Enable</a>
                @endif

                <form id="deletePlanForm" method="post" action="{!! url(config('siravel.backend-route-prefix', 'siravel').'/plans/'.$plan->id) !!}">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
