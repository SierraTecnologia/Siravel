...
@section('content')
    ...
    <div class="col-md-12 mt-4">
        ...
        {!! Form::model($order, ['route' => [config('siravel.backend-route-prefix', 'siravel').'.orders.update', $order->id], 'method' => 'patch']) !!}
            ...
            <div class="row">
                <div class="col-md-6">
                    <table class="table table-sitecpaymentd">
                        ...
                        <tr>
                            <td><a href="{{ url(config('siravel.backend-route-prefix', 'siravel').'/orders/item/'.$item->id) }}">{{ $item->product->name }}</a></td>
                            <td>{{ $item->quantity }}</td>
                            <td>{{ $item->total }}</td>
                            <td>{{ ucfirst($item->status) }}</td>
                            <td class="text-right">
                                <a href="{{ url(config('siravel.backend-route-prefix', 'siravel').'/orders/item/'.$item->id) }}" class="btn btn-sm btn-outline-primary">Review</a>
                            </td>
                        </tr>
                        @endforeach
                    </table>
        ...
        @if ($order->status !== 'cancelled' && !$order->hasRefundedOrderItems())
            {!! Form::open(['id' => 'cancelForm', 'url' => config('siravel.backend-route-prefix', 'siravel').'/orders/cancel', 'method' => 'post', 'class' => 'inline-form pull-left']) !!}
