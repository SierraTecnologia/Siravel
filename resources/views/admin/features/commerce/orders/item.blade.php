...
@section('content')
    ...
    <div class="col-md-12 mt-4">
        ...
        @include('siravel::admin.features.commerce.orders.breadcrumbs', ['location' => [['Order' => url(config('siravel.backend-route-prefix', 'siravel').'/orders/'.$orderItem->order_id.'/edit')], 'item']])
            ...
            <div class="col-md-6 raw-margin-bottom-24">
                <table class="table table-sitecpaymentd">
                    <tbody>
                        <tr>
                            <th>Product</th>
                            <td class="text-right"><a href="{!! route(config('siravel.backend-route-prefix', 'siravel').'.products.edit', [$orderItem->product_id]) !!}">{{ ucfirst($orderItem->product->name) }}</a></td>
                        </tr>
                        <tr>
                            <th>Quantity</th>
            ...
            <div class="col-md-6 raw-margin-bottom-24">
                ...
                @if (!$orderItem->was_refunded)
                    <div class="text-right">
                        {!! Form::open(['id' => 'cancelItemForm', 'url' => config('siravel.backend-route-prefix', 'siravel').'/orders/item/cancel', 'method' => 'post', 'class' => 'inline-form pull-right']) !!}
