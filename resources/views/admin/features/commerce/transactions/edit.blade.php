...
@section('content')
    ...
    <div class="col-md-12">
        <div class="row raw-margin-bottom-24">
            <div class="col-md-6 mt-2">
                @include('siravel::admin.features.commerce.transactions.breadcrumbs', ['location' => ['edit']])
            </div>
    ...
    {!! Form::model($transaction, ['route' => [config('siravel.backend-route-prefix', 'siravel').'.transactions.update', $transaction->id], 'method' => 'patch']) !!}
    ...
    <div class="col-md-12">
                ...
                @if ($order && $order->hasActiveOrderItems())
                    <div class="card bg-light border-dark">
                        <div class="box-header panel-heading card-header">
                           <a href="{{ url(config('siravel.backend-route-prefix', 'siravel').'/orders/'.$order->id.'/edit') }}">Order: #{{ $order->id }}</a>
                        </div>
                        <div class="card-body">
        ...
        <div class="form-group">
            ...
            @if ($order && is_null($transaction->refund_date))
                {!! Form::open(['id' => 'refundForm', 'url' => config('siravel.backend-route-prefix', 'siravel').'/transactions/refund', 'method' => 'post', 'class' => 'inline-form float-left']) !!}
