    ...
    <tbody>
        @foreach($transactions->reverse() as $transaction)
            <tr>
                ...
                <td class="text-right">
                    <a class="btn btn-sm btn-secondary float-right" href="{!! route(config('siravel.backend-route-prefix', 'siravel').'.transactions.edit', [$transaction->id]) !!}"><i class="fa fa-eye"></i> View</a>
                </td>
            </tr>
        @endforeach
    </tbody>
